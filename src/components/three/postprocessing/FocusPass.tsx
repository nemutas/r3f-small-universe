...
import { focusPassState } from '../../../modules/store';

export const FocusPass: VFC = () => {
	const passRef = useRef<ShaderPass>(null)

	// const gui = GUIController.instance.setFolder('Focus')
	// gui.addCheckBox(focusPassState, 'enabled')
	// gui.addNumericSlider(focusPassState, 'focus', -0.5, 0.5, 0.01)
	// gui.addNumericSlider(focusPassState, 'blur', 0, 1, 0.01)
	// gui.addNumericSlider(focusPassState, 'samples', 10, 100, 10)

	const shader: THREE.Shader = {
		uniforms: {
			tDiffuse: { value: null },
			u_focus: { value: focusPassState.focus },
			u_blur: { value: focusPassState.blur },
			u_samples: { value: focusPassState.samples }
		},
	...
	const update = () => {
		passRef.current!.enabled = focusPassState.enabled

		if (focusPassState.enabled) {
			passRef.current!.uniforms.u_focus.value = focusPassState.focus
			passRef.current!.uniforms.u_blur.value = focusPassState.blur
			passRef.current!.uniforms.u_samples.value = focusPassState.samples
		}
	}
